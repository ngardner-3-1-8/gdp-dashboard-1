...
def collect_schedule_travel_ranking_data(pd):
    ...
    def add_odds_to_main_csv():
        ...
        if mask.any():
            ...
            csv_df['Favorite'] = csv_df['Home Team'] if csv_df['Home Team Adjusted Current Rank'] >= csv_df['Away Team Adjusted Current Rank'] else csv_df['Away Team']
            csv_df['Underdog'] = csv_df['Home Team'] if csv_df['Home Team Adjusted Current Rank'] < csv_df['Away Team Adjusted Current Rank'] else csv_df['Away Team']
            ...
            csv_df['Home Team Moneyline'] = csv_df.apply(lambda row: odds[(round(csv_df['Adjusted Spread'] * 2) / 2)][0] if csv_df['Favorite'] == csv_df['Home Team'] else odds[(round(csv_df['Adjusted Spread'] * 2) / 2)][1], axis=1)
            csv_df['Away Team Moneyline'] = csv_df.apply(lambda row: odds[(round(csv_df['Adjusted Spread'] * 2) / 2)][1] if csv_df['Favorite'] == csv_df['Home Team'] else odds[(round(csv_df['Adjusted Spread'] * 2) / 2)][0], axis=1)
